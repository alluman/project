...
class updown:
    ...
    def result(self):
        if self.pnumber > self.answer:    
            self.max_range = self.pnumber - 1
            print('정답보다 큽니다.')
        elif self.pnumber < self.answer: 
            self.min_range = self.pnumber + 1
            print('정답보다 작습니다.')
        elif self.best_i is None or self.best_i > self.i:
            self.best_i = self.i
